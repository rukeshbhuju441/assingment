package bhujutakeoutassingment;

import java.util.Scanner;

public class BhujuTakeOut {
	...
	public static void main(String[] args) {
		...
		do {
			...
			case "1":
				BhujuTakeOut.displayItalianFoodMenu();
				int[] feedItalianFood = { 5, 7, 4 };
				double[] priceItalian = { 17.99, 15.99, 12.99 };
				int opt = BhujuTakeOut.foodChoice(feedItalianFood, priceItalian);
				BhujuTakeOut.finalPrice(feedItalianFood, priceItalian, opt, numberOfPeople);
				isWrongMenuChoice = false;
				break;

			case "2":
				BhujuTakeOut.displayChineseFoodMenu();
				int[] feedChineseFood = { 7, 7, 5 };
				double[] priceChinese = { 18.99, 18.99, 10.99 };
				opt = BhujuTakeOut.foodChoice(feedChineseFood, priceChinese);
				BhujuTakeOut.finalPrice(feedChineseFood, priceChinese, opt, numberOfPeople);
				isWrongMenuChoice = false;
				break;

			case "3":
				BhujuTakeOut.displayAmericanFoodMenu();
				int[] feedAmericanFood = { 8, 5, 10 };
				double[] priceAmerican = { 21.99, 22.99, 26.99 };
				opt = BhujuTakeOut.foodChoice(feedAmericanFood, priceAmerican);
				BhujuTakeOut.finalPrice(feedAmericanFood, priceAmerican, opt, numberOfPeople);
				isWrongMenuChoice = false;
				break;
			default:
	...
	public static void displayAmericanFoodMenu() {
		String[] americanFood = { "Hamburger and Hot Dog Tray - includes buns and condiments",
	...
	public static void finalPrice(int[] feeds, double[] prices, int choice, int guestCount) {
		int trays = BhujuTakeOut.determineTrays(guestCount, feeds[choice - 1]);
		double subTotal = BhujuTakeOut.getSubtotal(prices[choice - 1], trays);
		double tax = BhujuTakeOut.getTax(subTotal, .07);
		double tip = BhujuTakeOut.getTip(subTotal, .15);
		double grandTotal = BhujuTakeOut.getGrandTotal(subTotal, tax, tip);
		...
		System.out.printf("Price per person: $ %.2f\n", BhujuTakeOut.pricePerPerson(grandTotal, guestCount));
		System.out.println("Left-over servings for the delivery person: "
				+ BhujuTakeOut.determineLeftOvers(feeds[choice - 1], trays, guestCount));
	}
